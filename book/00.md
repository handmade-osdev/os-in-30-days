...
It is possible to build a unique PC by choosing and combining together different parts. It is also possible to create games and applications by yourself. If you use a website builder, you can create a custom website. And if you read the masterpiece called "How to make a CPU", you can even make your own CPU.

By the way isn't there something missing here? Right, it's the creation of your own OS. It doesn't feel like something that any beginner can make. Even though PC, Games, Web Pages, CPU all seem to be challenging for beginners, it's sad that only OS's are left out. Well, if they are, I'll undertake writing about it.
...
But no worries needed! I have the experience of developing a small OS, which doesn't even take up 80 Kb of space. Even though it's small, it's a properly-featured OS. It's not a console-only OS and doesn't omit multitasking.

If it's about 80KB, don't you think you can create something like this if you try apply some effort? Don't you feel like even a beginner could create something like this? Yes, it can be made in about a month! With that said let's take it easy.
...
As to what programming language we'll use, I think we'll go with C. Ah... I can hear the dissatisfied voices (bitter smile). Like "C is not good", "I want to do it in C++", "Java is good", "Let's rewrite it in rust", ... I understand your feelings, but for the ease of explanation please accept C. It doesn't have many functions, which makes it easy to use, so it is the right language for this book. If we used an other language, I'd have to first give the explanation of that language which is daunting.
...
For example, the total capacity of all floppy disks is 1440 KB, so the image file that is backup data is also 1440 KB. If you're free to create any backup image, it's possible to create discs with any content.
...
The readme file contains a link to a discord server where you can ask the questions.
...
From now on I will assemble the OS little by little, but as I progress a little the stages in the middle will be summarized. The stages are written down fully in the appendix, and if you copy it you can run it immediately. There are caveats in about the program so I would like to write about them.

For example the first program in the appendix is "days/01_v1", and the program that appears next is "days/01_v2". If you follow the instruction for the text for "v1", it will become "v2", wouldn't it? No it won't. That's because sometimes I do some changes behind the scenes. I just didn't explain it because I thought you would understand it without the explanation. By the way the days are named according to the day and the section within that day. This is why some of the v-numbers may be not contiguous.
...
Before we actually start developing our operating system, we'll need to set up the environment and learn to use a new way of interacting with operating system -- command line.

You may be used to the fact that when you run programs on Windows usually a Window with buttons, text fields, menus and pictures appears. These kinds of windows have what's called GUI (Graphical User Interface). But before the computers were advanced, the software couldn't afford fancy graphics with buttons and images, so these programs operated using text only. These programs have what's called CLI (Command Line Interface). CLI programs are run using a command line, a kind of program that lets you run text based commands.

Most of the tools that were created for OS development (compilers, image creation tools, assemblers etc.) are distributed as CLI software. That is the major reason I'm including the explanation of how to use the command line. Because otherwise you either wouldn't understand what's going when we build our OS, or you'll be forced to be on your own learning about command line, which is not fun.

The minor reason I include this explanation, is because in the world of programming the command line is widely used. By studying about how it works you will obtain one more useful skill that you may be able to apply elsewhere.

I will keep this chapter relatively short, only writing the necessary basics. We will learn more about various shell commands as we go, because that's better.

The command line is called `cmd.exe`. There are multiple ways to start it:

- Open start menu and search `cmd.exe`.

- Press `win+x`, and then in the menu choose "Command Prompt". You can also press the `c` key while the menu is open. (Win8+)

- Press `win+r`, and see the "run" window appear. Type "cmd" and hit enter.

- The executable path: `C:/Windows/System32/cmd.exe`

Choose your favourite way and open the command line. Now what you should see is something like this appear on the screen:

```
C:\Windows\System32> 
```

This line is called "prompt". The part before `>` symbol is the current directory. You can think of the current directory as the folder you're currently located in. The part after `>` is the place where you enter commands. Let's try typing some commands.

Now, the first command we will learn is a very simple command. It's called `echo`. It will print whatever you say it to print.
...
It printed whatever we told it to print (If it didn't you probably live in the future where computers took over the world. Fortunately I live in 2021, and computers are still more stupid than humans. Thank god!). You can print files in the current directory using the `dir` command.
...
In this case we printed all the files that are located in `C:\Windows\System32` directory. There is probably a lot of files. You can also print the files contained in another directory by typing it like `dir <path>`.
...
Typing out the prompt every time I want to mention a command is tiring so from now on I will avoid typing it. So repeating the last command, I will simply write `>` followed by the command:
...
Now I'll teach you how to create a folder. For this an `mkdir` command is used like this:
...
Make sure you are allowed to create folders in the current directory. If not, change the directory to something else. In the example above we first create the directory using the `mkdir` command. Then we use `dir` command to print list of files and verify that we indeed created a directory.

One more important command is `rmdir`. This command deletes empty directories.

```
> rmdir folder_name
```

This should delete `folder_name` from current directory. By the way if you find out that `folder_name` is not empty you can pass a `/s` option like that:
...
Now let's create a folder that we will use for the rest of our operating system experiments. I recommend you something like `C:\my_cool_os`, because the path to it is the shortest, and you won't have to end up remembering long strings of text.
...
From now on whenever I ask you to open command line I will expect that your current directory is in that folder. Make sure to `cd` into it whenever you start command line.
