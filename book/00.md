...
One more important thing. This will be our first practical tutorial on command lines.
...
Most of the tools that were created for OS development are distributed as CLI software. Compilers, image creation tools, assemblers etc. all have CLI. That is the major reason I'm including the explanation of how to use the command line. Because otherwise you either wouldn't understand what's going when we build our OS, or you'll be forced to be on your own learning about command line, which is not fun.
...
I expect the Linux users to already know their favourite shell and how to create command files. So Linux users should skip this section.
...
You can also print the files contained in another directory by typing it like `dir <path>`.
...
> REM now check it was created
> dir
...
folder_name
```

Make sure you are allowed to create folders in the current directory. If not, change the directory to something else. By the way lines that begin with REM are ignored.

For example you can create a folder at `C:\OS`.
...
Simple, right? Well, if you don't want to, you don't have to delete the OS folder. If you're following the and code along, you should have a folder where you'll be developing the OS. From now on I will assume that whenever I say "start the command line", you will change the current directory to that folder.
