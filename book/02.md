...
## Chapter 02 / Learning assembly and BIOS (day 2) {#ch02}

### 1. Resuming development {#ch02-01}

Yesterday I didn't explain the parts with the program and the rest of the boot sector.

Whenever you hit the start button of your PC, BIOS starts looking at the disks one by one, in the order defined by it's settings. If you ever installed an OS from USB stick you should be familiar with those. BIOS looks at the first 512 bytes of each disk, and if the last bytes (in other words bytes 510 and 511) are 0x55 and 0xaa, BIOS loads this disk into memory and starts executing the code, starting from the first byte.

In other words if the last bytes are NOT 0x55 0xaa, BIOS won't be able to boot from that disk. This explains the "rest of boot sector" part.

And for the code, we'll start with the example. In fact the assembly code for the program looks like this:
...
![](media/02_memory_vs_register.png)
...
In the code we used `CMP AL, 0` which means to compare AL with 0.
...
You can verify that everything works by running this image in the emulator again.
...
Makefiles are another common way of building complex programs, such as OS'es or big games. Makefiles have nothing to do with our `make.bat`, for one it's ran by a different program, not by the command line. In essense makefile is the same list of contiguous commands in a spacial command line -like language. The only difference is that makefiles have additional features that are helpful for saving time when building large projects. Why won't we use the more powerful makefiles then?
