...
Well it's easier to make the OS than to write about it, so we'll start development right now. And so, completely ignoring the explanation so far, I will start developing not in C, nor in assembler, but a different tool (lol).
...
So now to install the disk image onto USB. For Windows users: open Win32DiskImager. Choose the image file that you want to write, and make sure that the disk it writes to is the correct USB. Then click "write" and confirm. After some time the image should get written and you'll be notified.
...
So I decided to not only use the `DB` instruction but also the `RESB` instruction and made helloos.asm much shorter, but of course the result hasn't changed. Please have a look:
...
Typing it out might be a bit of a hassle, so I put this program into the day01_01 folder. By the way we don't need the day01_00 folder anymore so you may delete it, or keep it as a keepsake. By the way I made day01_01 folder by changing the name of day01_00 folder, removing unneeded files and adding the necessary files. That's how the folders are growing little by little.
...
This instruction is the ultimate strongest weapon in the world of assembler. That is, any data (and any program, by the way) can be written with just that instruction. That's why there are no files that can't be created in assembler. You can create text files, image files, whatever you're told to do, you can do it in assembler. This expressiveness is rare in other languages (e.g. C).
...
TN: The implication is that in NASM it will also reserve zeroes but, it will hesitate. While actually filling the file with zeroes it prints a warning like this:

```
helloos.asm:9: warning: uninitialized space declared in .text section: zeroing [-w+other]
```

To get rid of this warning add an option `-w-other` to the NASM command, or as an alternative to disabling various warnings, you can substitute `RESB N` for `TIMES N DB 0`. This will actually fill the file with zeroes.
...
I'm really happy with 22 lines, but it has no meaning included. So, I added a bit of meaning to it. The file has increased in length to 50 lines. You can find it in day01_02. Now you don't need the day01_01 folder. You can delete it or keep it as a keepsake.
...
For the new highlights, `;` marks a line comment, equivalent to `//` in C or C++. Thanks to this instruction you can add various comments into a source program.
